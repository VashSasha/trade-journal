-- Persist coach-only voice mute independently of coaching and master sound.
-- Apply after 0030, before deploying the floating coach frontend. Old clients preserve this preference.
-- No accounts, trades, entitlements or historical balances are modified.
begin;
...
begin
    ...
    if p_kind = 'session_schedule' then
        foreach rule_name in array array['asia', 'london', 'new-york'] loop
            ...
            if jsonb_typeof(rule_value) is distinct from 'object'
                ...
                or jsonb_typeof(rule_value->'openMinute') is distinct from 'number'
                or jsonb_typeof(rule_value->'closeMinute') is distinct from 'number' then
                raise exception 'Invalid session schedule: %', rule_name;
            end if;
            open_value := (rule_value->>'openMinute')::numeric;
            close_value := (rule_value->>'closeMinute')::numeric;
            if open_value <> trunc(open_value) or close_value <> trunc(close_value)
                or open_value < 0 or open_value >= 1440 or close_value < 0 or close_value >= 1440
                or open_value = close_value then
                raise exception 'Invalid session hours: %', rule_name;
            end if;
            normalized := normalized || jsonb_build_object(rule_name, jsonb_build_object(
                'enabled', (rule_value->>'enabled')::boolean,
                'openMinute', open_value::integer,
                'closeMinute', close_value::integer
            ));
        end loop;
    elsif p_kind = 'performance_alerts' then
        ...
        -- Opt-in is optional for old clients; omission preserves the account setting.
        if p_preferences->'dailyProfit' ? 'includeOpenPnl' and
            jsonb_typeof(p_preferences->'dailyProfit'->'includeOpenPnl') is distinct from 'boolean' then
            raise exception 'Invalid open-PnL preference';
        end if;
        normalized := jsonb_set(normalized, '{dailyProfit,includeOpenPnl}', coalesce(
            p_preferences->'dailyProfit'->'includeOpenPnl',
            (select prefs#>'{performance_alerts,dailyProfit,includeOpenPnl}'
             from public.user_settings where user_id = owner_id),
            'false'::jsonb
        ));
        ...
        if p_preferences ? 'voiceEnabled' and
            jsonb_typeof(p_preferences->'voiceEnabled') is distinct from 'boolean' then
            raise exception 'Invalid Coach voice preference';
        end if;
        normalized := jsonb_build_object(
            'voiceEnabled', coalesce(p_preferences->'voiceEnabled',
                (select prefs->'live_coach'->'voiceEnabled' from public.user_settings where user_id = owner_id),
                'true'::jsonb),
