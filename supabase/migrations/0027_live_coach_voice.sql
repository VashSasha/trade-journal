-- Portable voice selection; old clients retain the stored voice on preference saves.
-- Audio and spoken comments remain transient and are never stored in these tables.
begin;
...
begin
    ...
    if p_kind = 'performance_alerts' then
        ...
        if p_preferences ? 'voice' and (
            ...
            or p_preferences->>'voice' not in ('browser', 'marin', 'cedar')
        ) then
