-- Add portable Live Coach controls to the existing validated alert-settings RPC.
-- Spoken runtime state, browser voices and broker events are deliberately not stored.
begin;
...
comment on function public.set_my_account_alert_preferences(text, jsonb) is
    'Validates and atomically merges portable alert and Live Coach settings into the current user settings.';
