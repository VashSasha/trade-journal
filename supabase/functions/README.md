...
market prediction, streaming report generation). The Anthropic API key exists
...
- `stream-analysis` requests return Server-Sent Events (Anthropic wire format).
...
| Secret | Purpose |
|---|---|
| `ANTHROPIC_API_KEY` | Anthropic API key used for every Claude completion |
