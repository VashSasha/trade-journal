...
app.whenReady().then(() => {
    ...
    //
    // Covered APIs: Tradovate, Anthropic (Claude) Supabase Edge Functions (AI reports — the
    // function's CORS allowlist doesn't know app://localhost),Discord Cloudflare workers exchange.
    ...
    session.defaultSession.webRequest.onHeadersReceived(externalApiFilter, (details, callback) => {
        callback({
            responseHeaders: {
                ...
                'access-control-allow-headers': ['Content-Type, Accept, Authorization, apikey, x-client-info,  x-api-key, anthropic-version, anthropic-dangerous-direct-browser-access'],
